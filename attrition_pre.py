...
full2 = pd.concat([BusinessTravelDF, DepartmentDF, EducationFieldDF,
                   ...
                   full['DistanceFromHome'], full['NumCompaniesWorked']], axis=1)
# 样本特征数据
...
pre_attrition = model_lr.predict(pre_data)
pre_attrition = pre_attrition.astype('int')
#
# 提取数据
user_ID = full.loc[1176:, 'user_id']
pre_result = pd.DataFrame({'user_id': user_ID, 'Attrition': pre_attrition})
pre_result.to_csv('pre_attrition.csv', index=False)
